...
import { ref as databaseRef, equalTo, onValue, orderByChild, query } from "firebase/database";
...
function Home(props) {
	...
	useEffect(() => {
		...
		onValue(postsRef, (snapshot) => {
			...
			if ( currCountry ){
				const currCountryRef = query(postsRef, orderByChild('country'), equalTo(currCountry))
		...
		<div className="block flex-center-col">
			...
			<div className="profile-body app-body">
					...
					<Dropdown>
						...
						<Dropdown.Menu as={CustomMenu}>
							{countriesList.map((country) => (
								<Dropdown.Item onClick={()=>changeCurrCountry(country)} >{country}</Dropdown.Item>
							))}
						</Dropdown.Menu>
					</Dropdown>
					<Dropdown>
				...
				<div className="home-content">
					...
					<h1 className="home-feed-header">{currCountry? currCountry: `Home`}</h1>
					<button
